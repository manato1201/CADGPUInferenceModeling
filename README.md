...
| **可視輪郭（VISIBLE）検出率** | **80.5%** | Zバッファ隠線判定（1024×1024深度マップ、car.dxf実測） |
| **隠線（HIDDEN）比率** | 約19.5% | 前版は99.9%隠線→大幅改善 |
...
寸法誤差:    0mm（完全一致）
隠線判定:    Zバッファ法（深度画像1024×1024）で80.5%が可視に正しく分類
```
...
| v3 | Zバッファ深度画像（512×512） | 79.1% |
| **v4（現行）** | **Zバッファ深度画像（1024×1024、numba njit高速化）** | **80.5%** |

詳細な計測条件・高速化の結果は [docs/PERF_RESULTS.md](docs/PERF_RESULTS.md) を参照。
...
```
メッシュ全三角形をラスタライズ → 1024×1024の深度マップ生成（numba njitで高速化）
  ↓
各エッジを5点サンプリング
  ↓ 各点の深度 vs 深度マップを比較
...
GPU不使用・ラスタライズはnumba njitでコンパイル。精度向上の過程:
...
| **v4（現行）** | **Zバッファ深度画像（1024×1024、numba njit）** | **80.5%** |

ラスタライズ自体はnumba化により大規模メッシュ(5万頂点超)で約20倍高速化したが、
エッジ可視性判定・ezdxf書き出し処理が別のボトルネックのため、triview変換全体の
所要時間は大規模メッシュではまだ改善の余地がある。詳細は
[docs/PERF_RESULTS.md](docs/PERF_RESULTS.md) を参照。
...
| 隠線判定速度 | 大規模メッシュ(5万頂点超)でtriview変換に約50秒 | ラスタライズ自体はnumba化済み(約20倍高速)だが、エッジ可視性判定・ezdxf書き出しが別ボトルネック。詳細は[docs/PERF_RESULTS.md](docs/PERF_RESULTS.md) |
